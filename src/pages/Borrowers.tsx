    ...
    const columns = useMemo<ColumnConfig<[Borrower, number, number, number]>[]>(
        () => [
            ...
            { key: "phone", header: "Phone", width: "15%", render: ([b]) => <TooltipCell content={b.phone || "-"} /> },
            {
                ...
                header: "Total Loans",
                width: "12%",
                render: ([b, , total_loans]) =>
                    total_loans > 0 ? (
                        <Button variant="link" className="p-0 h-auto" asChild>
                            <Link to={`/loans?query=card:${b.card_id}`}>{total_loans}</Link>
                        </Button>
                    ) : (
                        <span>0</span>
                    ),
                ...
                render: ([b, , , fine]) =>
                    fine > 0 ? (
                        <Button variant="link" className="p-0 h-auto text-destructive" asChild>
                            <Link to={`/loans?query=card:${b.card_id} fine_is:owed`}>${fine.toFixed(2)}</Link>
                        </Button>
                    ) : (
            ...
            },
            {
                ...
                header: "Checkin",
                width: "16%",
                render: ([b, active_loans]) =>
                    active_loans > 0 ? (
                        <Button variant="outline" size="sm" asChild>
                            <Link to={`/loans?query=card:${b.card_id} loan_is:active`}>
        ...
        <div className="h-screen flex overflow-hidden">
            ...
            <main className="flex-1 p-10 overflow-y-scroll">
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <Button onClick={() => setIsModalOpen(true)}>New Borrower</Button>
                </div>
                <div className="mb-6">
                    <SearchArea
                        key={searchQuery}
